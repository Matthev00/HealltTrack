...
function ActivitiesForm({ onClose, onActivityAdded }: { onClose: () => void, onActivityAdded: () => void }) {
    ...
    const handleSaveActivities = (activityId: number, index: number) => {
        const Duration = duration[activityId];
        const StartHour = startHour[activityId] || '17:00'; // default value if not set
        const activityPerformed: activity_out = {
            user_id: 1,
            date: `${activitiesCtx.actualDate} ${StartHour}`, // include space between date and time
            activity_id: index + 101, // ensure this is the correct id
            duration: Duration,
            calories_burned: 0,
        };
        console.log("Activity to be saved:", activityPerformed);  // Log the activity data
        saveActivity(activityPerformed); // Save the activity on your server
        onClose();
        onActivityAdded(); // Call the callback function
    };
                ...
                {filteredActivitiesList.length > 0 && filteredActivitiesList.map((activity, index) => (
                    <div key={activity.id} className={`pb-4 ${index !== filteredActivitiesList.length - 1 ? 'mb-8 border-b border-gray5-300' : ''}`}>
                        <div className="flex justify-between">
                            ...
                            <button className="text-green-400" onClick={() => handleSaveActivities(activity.id, index)}>+</button>
                        </div>
